...
const gravityAccel = new Vector3(0, 0, -1 * gravityABS)
...
function simulateShot (fireAngleRad, muzzleVelocity, heightOfTarget, crossWind, tailWind, temperature, airDensity, airFriction) {
  return {finalPosX: 0, finalPosY: 0, currentTime: 0}
}
...
function simulatefindSolution (rangeToHit, heightToHit, muzzleVelocity, airFriction, minElev, maxElev, highArc) {
  ...
  if (!airFriction) {
    ...
    let radicand = Math.pow(muzzleVelocity, 4) - gravityABS * (gravityABS * Math.pow(rangeToHit, 2) + 2 * heightToHit * pow(muzzleVelocity, 2))
